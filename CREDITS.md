# Créditos y atribuciones

Este archivo contiene información sobre los autores y licencias de los assets utilizados en este videojuego.

---

## 🎮 Proyecto
**Nombre del juego:** QQ 

**Repositorio:** https://github.com/Qiu2025/FdV  

**Desarrolladores principales:**  
- Zhiwei Zhang (@javizhangg)
- Xiaolei Zhu (@MiiNeLoC0)
- Jiade Zheng (@jiade-git)
- Yixiao Yao (@MASKYX)
- Siyuan Qiu (@Qiu2025)
- Shuhang Pan (@Shuhang)

---

## 🎨 Arte y tilesets

- **Jugador (sprite principal)**  
  Autor: [duckhive](https://duckhive.itch.io)  
  Asset: [FREE Animated Pixel Art Penguin](https://duckhive.itch.io/penguin)  
  Licencia/uso: Uso libre para proyectos personales y comerciales. Ver página del autor.  
  Cambios: ajustes de color y recorte para el atlas del jugador.

- **Tileset de desierto (plataformas/objetos)**  
  Autor: [pzuh](https://pzuh.itch.io)  
  Asset: [Free Desert Platformer Tileset](https://pzuh.itch.io/free-desert-platformer-tileset)  
  Licencia/uso: Gratuito; revisar términos en la página del autor.  
  Cambios: reescalado a la cuadrícula del proyecto, recoloración de algunos objetos.

- **Tilesets estacionales (grassland/otoño/trópico/invierno)**  
  Autor: [GrafxKid](https://grafxkid.itch.io)  
  Asset: [Seasonal Tilesets](https://grafxkid.itch.io/seasonal-tilesets)  
  Licencia/uso: **CC0 (Creative Commons Zero 1.0)**. Crédito opcional.  
  Cambios: integración en paleta del proyecto; animaciones de agua/lava activadas.

- **Tileset de nieve (entorno nevado)**  
  Autor: [Free Game Assets ](https://free-game-assets.itch.io)
  Asset: [Snow Tileset Pixel Art](https://free-game-assets.itch.io/snow-tileset-pixel-art)  
  Licencia/uso: **Asset de pago** (CraftPix). Usado conforme a su licencia comercial; consultar términos en la página del asset.  
  Cambios: empaquetado en atlas y ajuste de colisiones.

---

## 🌅 Fondos y parallax

- **Fondo parallax de desierto**  
  Autor: [Cryptogene](https://cryptogene.itch.io)  
  Asset: [Free Parallax Desert Background](https://cryptogene.itch.io/parallax-desert-background)  
  Licencia/uso: Permitido para proyectos personales y comerciales (licencia no exclusiva); ver detalles en la página.  
  Cambios: capas reordenadas para la velocidad de scroll del juego.

- **Fondo “Desert landscape with dunes”**  
  Autor: [v.rozenfeld](https://vrozenfeld.itch.io)  
  Asset: [Desert landscape with dunes background](https://vrozenfeld.itch.io/desert-landscape-with-dunes-background)  
  Licencia/uso: “Name-your-price”; revisar términos en la página del asset.  
  Cambios: corrección de perspectiva y ligera edición de color.

---

## 🔊 Sonidos y música
*Actualmente no implementados*
---

## 🧩 Fuentes tipográficas
*Actualmente no implementados*
---


## 🧑‍⚖️ Nota legal

Todos los assets pertenecen a sus respectivos autores y se utilizan conforme a sus licencias.  
En caso de distribuir el juego, incluye este archivo y los avisos/licencias correspondientes.  
Si algún autor solicita un cambio en la atribución, actualizaremos este documento.

